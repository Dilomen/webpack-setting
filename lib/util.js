...
// 获取绝对位置
const getAbsPath = (relFilePath = '') => path.resolve(process.cwd(), relFilePath);
...
function createTemplete() {
    ...
    if (!tempExist) {
        try {
            ...
            fs.writeFileSync(path.resolve(__dirname, './index.html'), ejs.render(template, { CDN_CSS, CDN_JS }))
        } catch (err) {
