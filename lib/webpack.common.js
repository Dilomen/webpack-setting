const path = require('path')
const babelOption = require('./babel.config')
const package_json = require(path.resolve(process.cwd(), './package.json'))
const webpack = require('webpack')
const HtmlWebpackPlugin = require('html-webpack-plugin')
const { CleanWebpackPlugin } = require('clean-webpack-plugin')
const { config = {} } = package_json
const { rules = [], output = {}, plugins = [], fileEntry = "", providePlugin = {}, alias = {} } = config
const { createAliasObj, checkFileExist } = require('./util')
let tempExist = checkFileExist()
let commonOption = {
  //process.cwd()返回运行当前脚本的工作目录的路径
  entry: [path.resolve(process.cwd(), fileEntry || "./src/index.tsx")],
  output: {
    path: path.resolve(process.cwd(), './build'),
    filename: "[hash:8].js",
    ...output
  },
  // 方便调试
  resolve: {
    extensions: ['.js', '.jsx', '.ts', '.tsx', '.css', '.scss'],
    alias: {
      'src': path.resolve(process.cwd(), "./src"),
      'components': path.resolve(process.cwd(), "./src/components"),
      'utils': path.resolve(process.cwd(), "./src/utils"),
      'assets': path.resolve(process.cwd(), "./src/assets"),
      'common': path.resolve(process.cwd(), "./src/common"),
      'base': path.resolve(process.cwd(), "./src/base"),
      ...createAliasObj(alias)
    }
  },
  plugins: [
    new CleanWebpackPlugin(),
    new HtmlWebpackPlugin({
      filename: 'index.html',
      template: tempExist ? path.resolve(process.cwd(), './index.html') : path.resolve(__dirname, './index.html')
    }),
    // 自动加载模块，而不必到处 import 或 require
    new webpack.ProvidePlugin({
      React: 'react',
      ReactDOM: 'react-dom',
      PureComponent: ['react', 'PureComponent'],
      Component: ['react', 'Component'],
      Classnames: ['classnames'],
      ...providePlugin
    }),
    ...plugins
  ],
  module: {
    rules: [
      {
        test: /\.jsx?$/,
        exclude: /node_modules/,
        loader: 'babel-loader',
        options: {
          ...babelOption()
        }
      },
      {
        test: /\.tsx?$/,
        exclude: /node_modules/,
        loader: "ts-loader"
      },
      {
        test: /\.(jpe?g|png|woff|woff2|ttf|eot)$/,
        use: [
          {
            loader: 'url-loader',
            options: {
              limit: 10000
            }
          }
        ]
      },
      {
        test: /.svg$/,
        oneOf: [{
          resourceQuery: /inline/, // 可在svg文件后加?inline， import SVG from icon.svg?inline
          use: 'url-loader'
        }, {
          use: 'raw-loader'
        }]
      },
      ...rules
    ]
  }
}

module.exports = commonOption