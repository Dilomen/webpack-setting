...
function getNodeModules (dir, path) {
  return getLibAbsPath(dir, `../node_modules/${path}`)
}
...
function handleReactSetting (webpackChainOption) {
  webpackChainOption.plugin('ProvidePlugin').tap(args => [
    ...args,
    {
      React: 'react',
      ReactDOM: 'react-dom',
...
function handleTypeScriptsSetting (webpackChainOption) {
  webpackChainOption.module
    ...
    .end()
    .use('cache-loader')
    .loader(getNodeModules('cache-loader'))
    .end()
    .use('ts-loader')
    .loader(getNodeModules('ts-loader'))
    .options({
      configFile: getLibAbsPath(__dirname, './public/tsconfig.json')
    })
}
