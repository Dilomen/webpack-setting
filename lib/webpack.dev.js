...
if (checkFileExist(path.resolve(process.cwd(), "./webpack.dev.js"))) {
  const webpackConfig = require(path.resolve(process.cwd(), "./webpack.dev.js"));
  webpackConfig.changeDefault && (option = webpackConfig.changeDefault(option) || option)
  option = merge(option, webpackConfig.options)
}
